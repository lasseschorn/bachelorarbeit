...
satistic$Wert <- as.integer(satistic$Wert)
met1 <- 'neuronal_network1'
met2 <- 'rondom_forest_regression'
save_data <- str_c('~/../bachelorarbeit/versuchsreihe/statistics/',met1,'/' )
setwd(save_data)

ggplot(data = filter(satistic,Methode==met1,Metric == 'RMSE' , Wert!=0  )) +
...
#  facet_wrap(~ metric)
  ...
  ggplot(data = filter(satistic,Methode==met1, Metric == 'MAE', Wert!=0  )) +
    geom_line(mapping = aes(x =Missing, y = Wert, group=Variablenname, color=Variablenname),size=5)+
    geom_point(mapping = aes(x =Missing, y = Wert))+
    scale_y_log10(labels = scales::label_number_si())+
    theme(plot.title=element_text(size = 20, face="bold"))+
  ...
  ggplot(data = filter(satistic,Methode==met1, Metric == 'DSC', Wert!=0  )) +
    geom_line(mapping = aes(x =Missing, y = Wert, group=Variablenname, color=Variablenname),size=5)+
    geom_point(mapping = aes(x =Missing, y = Wert))+
    scale_y_log10(labels = scales::label_number_si())+
    theme(plot.title=element_text(size = 20, face="bold"))+

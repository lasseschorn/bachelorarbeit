...
setwd(save_data)
Epocheen_2 <- read.csv('~/../bachelorarbeit/versuchsreihe/statistics/epochen_8_2.csv',header = TRUE,dec=",", sep = ";")
...
save_data <- str_c('~/../bachelorarbeit/versuchsreihe/statistics/','neuronal_network','/','8/' )
setwd(save_data)


ggplot(data = filter(Epocheen_2,Type != "root_mean_squared_error") , aes(x=Epoche, y=Wert, group=Type)) +
  geom_smooth()+
...
ggsave("Epocheen_8_gesamt_loss.png", width = 25, height = 10)
  ...
  facet_grid(.~Missing)+
    ...
    theme(legend.title = element_blank())+#element_text(size = legen_size, face="bold"))+
    theme(legend.text = element_text(size = legen_size))
ggsave("Epocheen_8_gesamt_rmse.png", width = 25, height = 10)
...
ggplot(data =data , aes(x=Epoche, y=Wert, group=Type)) +
  ...
  theme(legend.title = element_blank())+#element_text(size = legen_size, face="bold"))+
  theme(legend.text = element_text(size = legen_size))
ggsave("Epocheen_8_gesamt.png", width = 25, height = 10)

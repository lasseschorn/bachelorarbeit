...
setwd(save_data)
Epocheen_2 <- read.csv('~/../bachelorarbeit/versuchsreihe/statistics/epochen_100_2.csv',header = TRUE,dec=",", sep = ";")
...
save_data <- str_c('~/../bachelorarbeit/versuchsreihe/statistics/','neuronal_network','/','100/' )
setwd(save_data)
...
ggsave("Epocheen_100_gesamt_loss.png", width = 25, height = 10)
...
ggplot(data =data , aes(x=Epoche, y=Wert, group=Type)) +
  ...
  theme(legend.title = element_blank())+#element_text(size = legen_size, face="bold"))+
  theme(legend.text = element_text(size = legen_size))
ggsave("Epocheen_100_gesamt_rmse.png", width = 25, height = 10)
...
ggplot(data =data , aes(x=Epoche, y=Wert, group=Type)) +
  ...
  theme(legend.title = element_blank())+#element_text(size = legen_size, face="bold"))+
  theme(legend.text = element_text(size = legen_size))
ggsave("Epocheen_100_gesamt.png", width = 25, height = 10)
